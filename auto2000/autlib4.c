...
/* MEH Added prefix to avoid conflicts with C-Interface to BLAS-1          */
/* function   auto_idamax : find index of element with max abs value    (BLAS-1)*/
...
dhhpr(integer k, integer j, integer n, doublereal *x, integer incx, doublereal *beta, doublereal *v)
{
  ...
  m = fabs(x[-1 + auto_idamax(jmkp1, &x[-1 + k], &incx)]);
