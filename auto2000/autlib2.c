  ...
  */
  ...
  if (ifst==1){
    ...
    */
    ...
    if(main_auto_storage.a!=NULL)free_dmatrix_3d(main_auto_storage.a);
    if(main_auto_storage.b!=NULL)free_dmatrix_3d(main_auto_storage.b);
    if(main_auto_storage.c!=NULL)free_dmatrix_3d(main_auto_storage.c);
    if(main_auto_storage.d!=NULL)free_dmatrix(main_auto_storage.d);
    if(main_auto_storage.a1!=NULL)free_dmatrix_3d(main_auto_storage.a1);
    if(main_auto_storage.a2!=NULL)free_dmatrix_3d(main_auto_storage.a2);
    if(main_auto_storage.s1!=NULL)free_dmatrix_3d(main_auto_storage.s1);
    if(main_auto_storage.s2!=NULL)free_dmatrix_3d(main_auto_storage.s2);
    if(main_auto_storage.bb!=NULL)free_dmatrix_3d(main_auto_storage.bb);
    if(main_auto_storage.cc!=NULL)free_dmatrix_3d(main_auto_storage.cc);
    if(main_auto_storage.ccbc!=NULL)free_dmatrix_3d(main_auto_storage.ccbc);
    if(main_auto_storage.faa!=NULL)free_dmatrix(main_auto_storage.faa);
    if(main_auto_storage.ca1!=NULL)free_dmatrix_3d(main_auto_storage.ca1);
    
    /* Free integer arrays */
    if(main_auto_storage.icf!=NULL)free(main_auto_storage.icf);
    if(main_auto_storage.irf!=NULL)free(main_auto_storage.irf);
    if(main_auto_storage.ipr!=NULL)free(main_auto_storage.ipr);
    if(main_auto_storage.icf11!=NULL)free(main_auto_storage.icf11);
    if(main_auto_storage.icf1!=NULL)free(main_auto_storage.icf1);
    if(main_auto_storage.icf2!=NULL)free(main_auto_storage.icf2);
    if(main_auto_storage.np!=NULL)free(main_auto_storage.np);
  ...
  if (ifst == 1) {
    ...
    setubv_make_boundary(ndim, ntst, nbc, nfpr, nrow, bcni, iap, rap, par, icp,
	   ...
	   ups, uoldps, dups);
    setubv(ndim, ips, ntst, ncol, nint, nalc,
...
*/
    ...
    setrhs(ndim, ips, ntst, ntst0, main_auto_storage.np, ncol, nbc, nint, nalc, useDefaultRHS, nfpr, nrc, nrow, &iam, &kwt, &ipar, funi, bcni, icni, ndxloc, iap, rap, par, icp, rds, fa, fc, rlcur, rlold, rldot, ups, uoldps, udotps, upoldp, dups, dtm, thl, thu);
  }
 ...
 if(!useDefaultRHS)
  {
   for(i=0;i<ntst;i++)for(j=0;j<ndim*ncol;j++)fa[i][j]=0.;
   for(i=0;i<nbc+nint;i++)fc[i]=0.;
   for(i=0;i<nalc;i++)fc[nbc+nint+i]=RHSVector[i];
  }
...
setrhs(integer ndim, integer ips, integer na, integer ntst, integer *np, integer ncol, integer nbc, integer nint, integer nalc, integer useDefaultRHS, integer ncb, integer nrc, integer nra, integer *iam, integer *kwt, logical *ipar, FUNI_TYPE((*funi)), BCNI_TYPE((*bcni)), ICNI_TYPE((*icni)), integer ndxloc, iap_type *iap, rap_type *rap, doublereal *par, integer *icp, doublereal *rds, doublereal **fa, doublereal *fc, doublereal *rlcur, doublereal *rlold, doublereal *rldot, doublereal **ups, doublereal **uoldps, doublereal **udotps, doublereal **upoldp, doublereal **dups, doublereal *dtm, doublereal *thl, doublereal *thu)
  ...
  *kwt = iap->numnodes;
  ...
  for (i = 0; i < nrc; ++i)fc[i] = 0.;

  /* Set constants. */
  ncp1 = ncol + 1;
  for (i = 0; i < ncb; ++i)par[icp[i]] = rlcur[i];
  ...
  for (jj = 0; jj < na; ++jj)
   {
	...
	for (l = 0; l < ncol; ++l)
      ...
      if (ips == 14 || ips == 16) rap->tivp = rlold[0];
  ...
  if (nint > 0)
   {
	...
	for (i = 0; i < ndim; ++i)
	  ...
	  uip[i] = upoldp[j1][i1];
        ...
        (*icni)(iap, rap, ndim, par, icp, nint, uic, uio, uid, uip, ficd, 0, dicd);
	for (m = 0; m < nint; ++m)
...
brbd(doublereal ***a, doublereal ***b, doublereal ***c, doublereal **d, doublereal **fa,
     ...
     integer *icf, integer useDefaultRHS, doublereal *RHSVector)
 {
  doublereal *fcc;
...
#endif
    reduce(iam, kwt, par, a1, a2, bb, cc, &d[nbc], na, nov, ncb, nrc-nbc, s1, s2, ca1, icf1, icf2, icf11, ipr);
#ifdef USAGE
  ...
  if (nllv == 0)
   {
    redrhs(iam, kwt, par, a1, a2, cc, faa, &fc[nbc], na, nov, nrc-nbc, ca1, icf1, icf2, icf11, ipr);
   }

  fcc = (doublereal *)malloc(sizeof(doublereal)*((nov + nrc) + (2*nov*nov)+1));

  dimrge(iam, kwt, par, cc, ccbc, d, fc, na, nrc, nbc, nov, ncb, idb, nllv, fcc, p0, p1, det, s1, a2, faa, bb,useDefaultRHS,RHSVector);
...
/*     ---------- ------ */
/* Subroutine */ int 
dimrge(integer iam, integer kwt, logical par, doublereal ***cc, doublereal ***ccbc, doublereal **d, doublereal *fc, integer na, integer nrc, integer nbc, integer nov, integer ncb, integer idb, integer nllv, doublereal *fcc, doublereal **p0, doublereal **p1, doublereal *det, doublereal ***s, doublereal ***a2, doublereal **faa, doublereal ***bb, integer useDefaultRHS, doublereal *RHSVector)
{
  ...
  if (iam == kwt - 1) {
    ...
    if (nllv == 0) {
      ...
      nlvc(ncrloc, ncrloc, nllv, 1, NULL, e, fcc);
    } else{
