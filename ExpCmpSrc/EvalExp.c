...
/* Check input */
 ...
 Register=malloc(object->nRegisters*sizeof(double));
 if(Register==(double*)NULL)
  {
   if(ECPrintErrorMessages)fprintf(ERROUT,"ECEvaluateExpression: Could not allocate %d bytes\n",object->nRegisters*sizeof(double));
   ECPrintSymbolTable(object);
   *rc=EC_IDENTIFIERS_NOT_SET;
#ifdef FLT_QNAN
   return(FLT_QNAN);
...
if(Destination>object->nRegisters)
 {
  printf("Destination (%d) > nRegisters (%d)\n",Destination,object->nRegisters);fflush(stdout);
  Register=realloc((void*)Register,(Destination+1)*sizeof(double));
  object->nRegisters=Destination+1;
 }
 ...
 *rc=EC_NO_ERROR;
 ...
 result=Register[(*object).returnRegister];
 free(Register);
 return result;
}
#ifdef __cplusplus
